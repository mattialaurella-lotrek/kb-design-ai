...
**T9 · Claude Design:** due fonti ufficiali pronte, una di assistenza e una del designer che lo ha costruito, e la decisione da prendere per prima è il confine con `T3`. La proposta è che `T3` tenga `/design` e il lavoro nel terminale, e `T9` il prodotto, cioè la tela di `claude.ai/design`, il design system che il team carica e fa approvare da un amministratore, l'export in PDF, PPTX, HTML e slide e la libreria di componenti tenuta in pari col repository da `/design-sync`. Se il confine non regge alla stesura, le due voci diventano una sola e la chiave che resta è `T3`. Due cose vanno ricontrollate il giorno che si scrive, perché la pagina dell'assistenza non porta una data e perché il prodotto è in beta.

**T4 · La ricerca UX con l'AI:** quattro fonti in attesa, tutte aperte, e due tengono il materiale che conta fuori dalla pagina, cioè un PDF di cinque pagine da scaricare e la parte dietro l'abbonamento della quarta, arrivata il 22 settembre 2026. Quella quarta dà al tema la spina che gli mancava, cioè la divisione fra il lavoro meccanico, che si delega, e quello di giudizio, che resta a chi conduce la ricerca, più i compiti ibridi che si spezzano in due. Due tagli si tengono, l'attrezzatura e il metodo, e il tema si colloca un gradino prima di «UX.md», perché quella sezione dice come le evidenze diventano contesto e questa direbbe come si producono.

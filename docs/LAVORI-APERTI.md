...
**Ogni voce ha una chiave,** da `I1` a `I9` per le integrazioni e da `T2` a `T9` per i temi, perché `T1` e `T10` sono diventati testo il 24 settembre 2026 e una chiave chiusa non si riusa, e la stessa chiave apre la sezione corrispondente in `docs/FONTI-DA-INTEGRARE.md`. I due file hanno la stessa spina dal 21 settembre 2026, quindi per sapere su cosa si regge un lavoro basta aprire là la sezione con la stessa chiave. Un titolo si riscrive e una chiave no, ed è per questo che c'è.
...
**Cosa ne esce, al 24 settembre 2026.** Sei integrazioni si possono scrivere senza aspettare niente, cioè `I1`, `I2`, `I4`, `I5`, `I6` e `I9`. Due sono ferme sulla stessa cosa, una lettura, e sono `I7` e `I8`: si recuperano tutte e due da `freedium-mirror.cfd`, che è la strada già collaudata, quindi conviene prenderle in un giro solo. Su `I8` una parte della risposta può arrivare dalla fonte nuova di `T8`, che tratta una skill della stessa famiglia. `I3` è letta e ferma su altro, cioè autore e data che la stampa dal mirror non porta. Fra i temi, `T1` e `T10` sono diventati testo il 24 settembre 2026, il più ricco è `T2` con otto fonti e tre verifiche da fare prima, e l'unico fermo per mancanza di materiale è `T7`, che aspetta le fonti dall'utente. I due arrivati il 22 settembre 2026, `T8` e `T9`, hanno le fonti in casa e aspettano una decisione invece di una lettura, cioè dove va la design critique e se Claude Design è una sezione sua o la stessa di `T3`.
...
**I1 · L'economia del contesto e dei token:** va in «Il contesto è una risorsa finita» e «Mantenere il contesto nel tempo», su quattro fonti già smistate. La decisione aperta è se le prime due diventano una sottosezione nuova o due paragrafi dentro quelle che ci sono, e si prende leggendole insieme, perché si sovrappongono. Dal 21 settembre 2026 dopo «Il contesto è una risorsa finita» c'è «I tre ruoli del contesto», e dal 24 settembre fra le due c'è anche «Il RAG nei progetti di Claude», quindi una sottosezione nuova allungherebbe una sequenza che è già di tre sezioni. Da questo giro esce anche una riga per «I comandi di Claude Code», dove `/config` non è in tabella. Attenzione a due dati che la stampa dell'articolo su Concise non conferma, cioè il campo `outputStyle` in `settings.json` e la versione 2.1.237, che vanno ripresi dalla documentazione ufficiale o tolti.
...
**I7 · L'audit del design system con Claude Code:** fermo perché non è ancora stato letto, e la collocazione dipende da cosa dice. Dal 24 settembre 2026 «Rendere il design system leggibile dall'AI» apre «Da dove partire» con un inventario di quello che nel sistema non va, preso da Romina Kavcic, quindi l'articolo va letto contro quel paragrafo e porta solo quello che manca. Il titolo della pagina parla di design system in Figma e l'indirizzo solo di design system, quindi il pezzo appartiene al capitolo su Figma nel primo caso e a «Rendere il design system leggibile dall'AI» nel secondo. Si recupera da `freedium-mirror.cfd`, la strada che ha funzionato per gli altri quattro articoli Medium.
...
Sono le otto voci di «Prossimi argomenti», in ordine di quanto sono pronte, quindi le chiavi non scendono in fila.

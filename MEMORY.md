...
- **Badge di stato sui titoli**: la sintassi `{badge:Testo}` a fine heading (gestita in `build.mjs`) genera una pill lime accanto al titolo e un pallino accanto alla voce nell'indice laterale (classe `flag`), lasciando pulito il testo del TOC. In uso su «UX-context design» con l'etichetta «In lavorazione»; da togliere quando la sezione si assesta.

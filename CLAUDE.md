...
- **Uno schema si fa con la famiglia `flow`,** non con un'immagine e non con SVG a mano: `flow` più `flow-2` o `flow-3` per le colonne, più `flow-steps` solo dove c'è un ordine e la freccia dice il vero. La famiglia è descritta in `DESIGN.md` sotto «Lo schema a riquadri».
- **La didascalia di una figura è un titolo stringato più il link alla fonte,** separati da un punto mediano, e niente frasi: `Globale, locale e di sfondo · Nielsen Norman Group`. Il titolo dice cosa mostra il disegno e non ripete il titolo della sezione. È l'unico posto dove autore e testata compaiono fuori dalle fonti.
- **Le fonti non si citano dentro il testo,** perché autore e testata in mezzo a un ragionamento fanno perdere il filo. Le fonti hanno la loro sezione e `docs/FONTI.md`. Il divieto riguarda la firma di un'opinione, non il soggetto di un fatto: `I comandi li ha scritti Anthropic` resta, `che NN/g distingue così` si toglie.
...
- **Le voci del catalogo di skill hanno tutte lo stesso peso,** cioè intorno alle 150 battute di testo visibile, URL escluso, con 240 come tetto. La mediana delle sessantasei già scritte è 155, e quattordici superano il tetto, fra 241 e 511 battute: sono debito, e si accorciano quando si mette mano alla loro riga. Una voce lunga il triplo delle vicine dice al lettore che quella skill conta di più, e non è mai quello che vogliamo dire. Se una skill merita davvero più spazio, il posto è il testo della guida e non l'elenco.
- **Le voci di «Prossimi argomenti» hanno tutte la stessa forma,** cioè cos'è il tema in una frase, cosa dirà la sezione e un dato di stato solo dove serve, come una funzione in prova o in beta. Si scrivono in forma impersonale, senza metafore e senza iperboli, e non passano le 80 parole. Il rimando a una sezione che esiste resta dove porta informazione, per esempio dove la guida dice già qualcosa sul tema.
- **«Stare» non dice dove si trova una cosa.** La collocazione in italiano vuole «si trova», «è» o «compare», quindi `sta in «Titolo»` diventa `si trova in «Titolo»`, e dove la frase prescrive dove mettere qualcosa si usa `va in`. Con «stare» restano soltanto gli idiomi, cioè stare in piedi, stare bene, starci dentro nel senso di entrarci, e il senso di consistere, come in «la differenza sta nella sola aggiunta».
- **Nemmeno «vivere» dice dove si trova una cosa.** Un file non vive nel repository e una skill non vive in `~/.claude`, quindi `vive in` diventa `si trova in`, `si colloca in` o `è in`. Dove il soggetto è un programma si usa `gira in` o `lavora in`, e dove la frase dice che qualcosa resta dov'è si usa `resta`.
